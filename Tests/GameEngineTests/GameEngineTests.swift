...
private final class FakeDisplayTransport: DisplayTransport {
    ...
    func send(_ message: ClientMessage) async {
        ...
        case let .loadResource(requestId, _, kind, _, preferredHandle):
            ...
            let response = Response(requestId: requestId, status: .ok, body: .image(handle: preferredHandle ?? 1, size: Size(24, 24)))
            await client?.receive(.response(response))
        case let .sendFrame(clientTick, _, cmds): frames.append((clientTick: clientTick, cmds: cmds))
        default: break
        }
    }

...
final class TennisRenderer {
    ...
    func loadResources(using client: DisplayClient) async throws {
        ...
        guard case let .font(handle, _) = body else {
            throw GenericError("Tennis font resource response was not a font for \(fontURL.absoluteString)")
        }
        fontHandle = handle
    }
    ...
    func draw(flow: TennisFlowState, match: TennisMatchSnapshot?, presentation: TennisPresentationState, using renderer: DisplayRenderClient) throws {
        ...
        case .result: try drawResult(flow: flow, using: renderer)
        }
    }

    private func drawTitle(using renderer: DisplayRenderClient) throws { try drawText("TENNIS", at: TennisPoint(x: 48, y: 48), using: renderer); try drawText("PRESS A", at: TennisPoint(x: 48, y: 72), using: renderer) }
    private func drawSurfaceSelect(flow: TennisFlowState, using renderer: DisplayRenderClient) throws { try drawText("SURFACE", at: TennisPoint(x: 48, y: 40), using: renderer); try drawText(String(describing: flow.highlightedSurface).uppercased(), at: TennisPoint(x: 48, y: 64), using: renderer); try drawText("LEFT / RIGHT / A", at: TennisPoint(x: 32, y: 88), using: renderer) }
    private func drawMatch(snapshot: TennisMatchSnapshot, presentation: TennisPresentationState, using renderer: DisplayRenderClient) throws { drawCourt(surface: snapshot.surface, using: renderer); try drawHUD(snapshot: snapshot, using: renderer); drawPlayersAndBall(snapshot: snapshot, using: renderer); drawCues(presentation, using: renderer) }
    private func drawResult(flow: TennisFlowState, using renderer: DisplayRenderClient) throws { try drawText(flow.result == .human ? "YOU WIN" : "CPU WINS", at: TennisPoint(x: 48, y: 56), using: renderer); try drawText("PRESS A", at: TennisPoint(x: 48, y: 80), using: renderer) }
    ...
    private func drawHUD(snapshot: TennisMatchSnapshot, using renderer: DisplayRenderClient) throws { try drawText("\(snapshot.score.human) - \(snapshot.score.cpu)", at: TennisPoint(x: 56, y: 8), using: renderer) }
    ...
    private func drawText(_ text: String, at point: TennisPoint, using renderer: DisplayRenderClient) throws { guard let fontHandle else { throw GenericError("Tennis text font is unavailable for content '\(text)'") }; renderer.drawCmd(DrawCmd(animationId: 0, parentAnimationId: 0, dest: Rect(x: point.x, y: point.y, width: 100, height: 16), color: .white, alpha: 1, z: 10, rotation: 0, rotationPoint: .zero, clippingRect: .zero, flip: [], time: 0, type: .text(fontHandle: fontHandle, content: text, size: 12, align: .left))) }

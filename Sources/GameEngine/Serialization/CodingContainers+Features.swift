...
public extension KeyedEncodingContainer where Key : CodingKey {
    ...
    mutating func encodeArray<T>(_ value: ContiguousArray<T>, forKey key: KeyedDecodingContainer<Key>.Key) throws {
        var container = self.nestedUnkeyedContainer(forKey: key)
        for eachItem in value {
            try container.encodeElementInArray(eachItem)
        }
...
public extension KeyedDecodingContainer where Key : CodingKey {
    ...
    func decodeContiguousArray<T>(_ type: T.Type, forKey key: KeyedDecodingContainer<Key>.Key) throws -> ContiguousArray<T> {
        return ContiguousArray<T>(try decodeArray(type, forKey: key))
    }
    
    func decodeContiguousArray<T>(_ type: T.Type, forKey key: KeyedDecodingContainer<Key>.Key) throws -> ContiguousArray<T> where T : Decodable {
        return ContiguousArray<T>(try decodeArray(type, forKey: key))
    }
